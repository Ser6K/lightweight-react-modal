...
//import * as ModalRegister from 'src/utils/register'
...
describe('Modal snapshots', () => {
  ...
  test('modal matches snapshot when maxHeight/minHeight/maxWidth/minWidth props are provided', () => {
    const modal = mount(
      <Modal minHeight={50} maxHeight={100} minWidth={60} maxWidth={120}>
        Modal
      </Modal>
    )
    expect(modal).toMatchSnapshot()
...
describe('Modal props', () => {
  ...
  test("onClose doesn't fire up on overlay click if dialog is not closable", () => {
    const onClose = jest.fn()
    const modal = mount(
      <Modal closable={false} onClose={onClose}>
        Modal
      </Modal>
    )
  ...
  test('close button default icon can be changed', () => {
    const onClose = jest.fn()
    const modal = mount(
      <Modal onClose={onClose} closeButtonIcon="Close">
        Modal
      </Modal>
    )
